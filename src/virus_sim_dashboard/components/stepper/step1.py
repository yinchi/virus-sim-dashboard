...
def layout() -> Generator[DashComponent, None, DashComponent]:
    ...
    with dmc.Stack(None, id="step1-stack", gap="md", m=0, p=0) as ret:
        yield dmc.Text(
            "Step 1 contents go here.",
        )
    return ret
...
@callback(
    ...
    Input("step1-stack", "id"),  # Dummy input to differentiate callbacks
...
def step1_on_next(
    ...
    # Only proceed if we are on step 1 (active_step == 0)
    if active_step != 0:
