...
def layout() -> Generator[DashComponent, None, DashComponent]:
    ...
    with dmc.Stack(None, id="step2-stack", gap="md", m=0, p=0) as ret:
        yield dmc.Text(
            "Step 2 contents go here.",
        )
    return ret
...
@callback(
    ...
    Input("step2-stack", "id"),  # Dummy input to differentiate callbacks
...
def step2_on_next(
    ...
    # Only proceed if we are on step 2 (active_step == 1)
    if active_step != 1:
