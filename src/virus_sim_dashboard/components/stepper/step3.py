...
def layout() -> Generator[DashComponent, None, DashComponent]:
    ...
    with dmc.Stack(None, id="step3-stack", gap="md", m=0, p=0) as ret:
        yield dmc.Text(
...
@callback(
    ...
    Input("step3-stack", "id"),  # Dummy input to differentiate callbacks
...
def step3_on_next(
    ...
    # Only proceed if we are on step 3 (active_step == 2)
    if active_step != 2:
